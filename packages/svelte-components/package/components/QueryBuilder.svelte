<script >import { buildClientSchema, isInputObjectType, isObjectType, isScalarType, Kind, isInterfaceType, GraphQLUnionType } from 'graphql';
export let introspectionQuery;
export let fieldName;
export let schemaField;
export let removeSelf;
import QueryArgumentEditor from './QueryArgumentEditor.svelte';
const createSelectionNode = (name) => ({
    kind: Kind.FIELD,
    name: {
        kind: Kind.NAME,
        value: name
...
const resolvesTo = (type) => 'ofType' in type ? resolvesTo(type.ofType) : type.name;
const resolvesToScalarType = (type) => 'ofType' in type ? resolvesToScalarType(type.ofType) : isScalarType(type);
export let ast;
let updateAst = (update) => {
    ast = update(ast);
};
const schema = buildClientSchema(introspectionQuery);
export let typeName = schema.getQueryType().name;
...
$: argMap = schemaField && schemaField.args.reduce((acc, arg) => {
    acc[arg.name] = arg;
    return acc;
}, {});
...
export let setDocumentFocus = (v) => {
    isDocumentFocus = true;
};
$: handleUpdateSelectionSet = (update) => {
    updateAst(ast => ({
        ...ast,
...
$: generateChildProps = (selectionNode) => {
