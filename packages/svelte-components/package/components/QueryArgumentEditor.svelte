<script >import { GraphQLEnumType, GraphQLInputObjectType, GraphQLList, GraphQLNonNull, GraphQLScalarType, Kind } from 'graphql';
const createArgumentNodeFromSchema = (name, graphQLArgument) => ({
    kind: Kind.ARGUMENT,
    name: {
        kind: Kind.NAME,
        value: name
...
const createFieldNodeFromSchema = (name, graphQLArgument) => ({
...
const createArgumentValueNodeFromSchema = (graphQLArgument) => {
...
let handleAddArgument = (parent, name, graphQLArgument) => {
